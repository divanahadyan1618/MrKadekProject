...
## 12. Future Work: Drift-Based Decision-Support Framework

### 12.1 Rationale

The current project does not implement drift detection, trigger states, management recommendations, or investment recommendations. Those capabilities belong in a future decision-support layer. The reason to document them here is methodological: the cleaned and scored outputs produced by the current workflow are suitable inputs for a later monitoring system. A future version should not only describe guest sentiment; it should indicate when management should pay attention. In operational terms, the key question would be:

If the rating or sentiment of reviews drifts away from the mean, median, or rolling baseline over a specified number of periods, what should management do?

This question requires a control-system perspective that has not yet been coded. A single negative review may be noise. A repeated decline across periods is more likely to signal a real change in guest experience, expectation alignment, or operational consistency.

### 12.2 Core Monitoring Metrics

A future monitoring module should compute, for each period:

- Mean star rating.
- Median star rating.
- Mean AFINN sentiment.
- Median AFINN sentiment.
- Mean Syuzhet sentiment.
- Percentage of reviews rated 1 to 3.
- Percentage of negative Syuzhet-classified reviews.
- Mean aspect ratings for value, rooms, location, cleanliness, service, and sleep quality.
- NRC emotion totals or rates, especially anger, disgust, fear, sadness, joy, and trust.
- Review count for the period.

The review count should be treated as a required confidence input. A future period with two reviews should not trigger the same response as a period with fifty reviews unless the content indicates a severe risk.

### 12.3 Baseline Definitions

A future implementation could define the baseline in three ways:

1. Rolling baseline: the mean or median over the previous k periods.
2. Prior cumulative baseline: all previous periods before the current one.
3. Seasonal baseline: the same month or quarter in prior years.
...
### 12.4 Drift Calculation

For a future metric M in period t:
...
Where enough historical data exist, drift could be standardized:

```text
standardized_drift_t = (M_t - baseline_t) / historical_variability
```

Historical variability can be measured using standard deviation or a robust measure such as median absolute deviation. Robust measures are preferable when the distribution is skewed or contains outliers.

### 12.5 Persistence Rule
...
In a future implementation, each period should be assigned a confidence label:
...
Pattern-to-action mapping is not currently implemented in the repository. It is a proposed future layer that could be developed after drift metrics and trigger states are coded. A future drift framework would become useful when different patterns are mapped to different action pathways.
...
If quantitative drift detection is implemented, it should always be followed by qualitative reading. The recommended future process is:
...
6. Implement drift metrics that compare rating, sentiment, low-rating share, negative-share, and aspect ratings against rolling baselines.
...
This methodology transforms TripAdvisor reviews of Bvlgari Resort Bali into a structured research dataset and visualization workflow. It begins with data validation and text preprocessing, applies multiple lexicon-based sentiment and emotion measures, compares text sentiment with star and aspect ratings, and visualizes sentiment patterns over time. The current project does not yet implement management or investment decision logic. Its future methodological contribution would be a drift-based decision-support extension in which rating and sentiment indicators are monitored against rolling baselines across consecutive periods, and persistent unfavorable drift is mapped to tiered management and investment responses.
